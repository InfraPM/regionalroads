class WfstLayer {
	...
	getFeatureType() {
		...
		var geometryTypes = ["gml:PointPropertyType", "gml:MultiPointPropertyType", "gml:LineString", "gml:MultiLineString", "gml:Polygon", "gml:MultiPolygon", "gml:MultiSurfacePropertyType", "gml:MultiCurvePropertyType"]
		for (var i = 0; i < a.length; i++) {
			var node = a[i];
	...
	setFidField() {
		...
		for (var i = 0; i < a.length; i++) {
			var node = a[i];
			if (node.attributes.name.value.toLowerCase() == 'id' || node.attributes.name.value.toLowerCase() == 'objectid') {
...
<token>` + this.appToken.token + `</token>
  <wfs:Delete typeName="`+ typeName + `">
	...
	getForeignKeyDropDown(fkJson, fieldName, defaultValue = null, required) {//generate a dro down list based on the foreign keys of the dataset
		var dropDownHTML = '<select id="' + fieldName + '" name="' + fieldName + '" ' + required + '>';
		for (var i = 0; i < fkJson.length; i++) {
			if (fkJson[i]['primaryColumnName'] == fieldName) {
	...
	getPopupForm() {//generate the Add Feature popup form
		var msg = this.describe;
		var fkJson = this.foreignKeyList;
		var htmlForm = '<h2>Add Feature - ' + this.displayName + '</h2><form id="addAttributesForm" action="nada" onsubmit="return false">';
		var xmlDoc = msg;
		var s = new XMLSerializer();
		var a = xmlDoc.getElementsByTagName("xsd:element");
		for (var i = 0; i < a.length; i++) {
			...
			if (s.serializeToString(node.attributes.name) != this.name) {
				...
				if (this.foreignKeyField(fkJson, curName) == false) {
					...
					if (formType == "hidden") {
						htmlForm += '<input type="' + formType + '" id="' + curName + '" name="' + curName + '" ' + curRequired + '>';
					}
					else if (formType == "textarea") {
						htmlForm += '<textarea id="' + curName + '" name="' + curName + '" rows="5" cols="45"' + curRequired + '></textarea><br>'
					}

					else {
						htmlForm += '<input type="' + formType + '" id="' + curName + '" name="' + curName + '" ' + curRequired + '><br>';
					}
				}
				else {
	...
	buildPointFeature(editLayer, formArray, mode) {
		...
		var gmlFeature = "";
		gmlFeature += '<gml:Point srsName="http://www.opengis.net/gml/srs/epsg.xml#' + this.projection + '">';
		var commaCount = 0
		var latlngString = "";
	...
	buildMultiPointFeature(editLayer, formArray, mode) {
		...
		var gmlFeature = "";
		gmlFeature += '<gml:MultiPoint srsName="http://www.opengis.net/gml/srs/epsg.xml#' + this.projection + '">';
		editLayer.eachLayer(function (layer) {
			if (mode == "Insert") {
				...
				else {//addToFeature mode
					var latlngString2 = "";
					gmlFeature += '<gml:pointMember>';
					gmlFeature += '<gml:Point>';
					gmlFeature += '<gml:coordinates decimal="." cs="," ts=" ">';
					latlngString2 = layer[latLngIndex]['lng'] + "," + layer[latLngIndex]['lat'];
	...
	buildLineFeature(editLayer, formArray, mode) {
		...
		var gmlFeature = "";
		gmlFeature += '<gml:LineString srsName="http://www.opengis.net/gml/srs/epsg.xml#' + this.projection + '">';
		var commaCount = 0
		var latlngString = "";
		gmlFeature += '<gml:coordinates decimal="." cs="," ts=" ">';
	...
	buildMultiLineFeature(editLayer, formArray, mode) {
		...
		var gmlFeature = "";
		gmlFeature += '<gml:MultiLineString srsName="http://www.opengis.net/gml/srs/epsg.xml#' + this.projection + '">';
		editLayer.eachLayer(function (layer) {
			if (mode == "Insert") {
	...
	buildPolygonFeature(editLayer, formArray, mode) {
		...
		var gmlFeature = "";
		gmlFeature += '<gml:Polygon srsName="http://www.opengis.net/gml/srs/epsg.xml#' + this.projection + '">';
		var commaCount = 0
		var latlngString = "";
	...
	buildMultiPolygonFeature(editLayer, formArray, mode) {
		...
		var gmlFeature = "";
		gmlFeature += '<gml:MultiPolygon srsName="http://www.opengis.net/gml/srs/epsg.xml#' + this.projection + '">';
		var addToFeature = false;
		editLayer.eachLayer(function (layer) {
			if (mode == "Insert") {
				gmlFeature += '<gml:Polygon>';
					...
					for (var k = 0; k < loopFeature[j].length; k++) {
						if (Array.isArray(loopFeature[j][k])) {
							...
							for (var l = 0; l < loopFeature[j][k].length; l++) {
								...
								latlngString += loopFeature[j][k][l]['lng'] + "," + loopFeature[j][k][l]['lat'];
								if (l == 0) {
									firstPoint = loopFeature[j][k][l]['lng'] + "," + loopFeature[j][k][l]['lat'];
								}
								spaceCount += 1;
							}
							...
							latlngString += loopFeature[j][k]['lng'] + "," + loopFeature[j][k]['lat'];
							if (k == 0) {
								var firstPoint = loopFeature[j][k]['lng'] + "," + loopFeature[j][k]['lat'];
							}
							spaceCount += 1;
						}
					}
	...
	buildGMLAttributes(mode, formArray) {
		...
		for (var i = 0; i < formArray.length; i++) {
			var skip = false;
			if (mode == "Insert") {
				if (formArray[i]['name'] != this.fidField && formArray[i]['name'] != this.geometryField) {
					var regex = RegExp('<\/?[a-z][\s\S]*>');
					var result = regex.test(formArray[i]['value']);
					if (formArray[i]['name'].toLowerCase().includes("date") && (formArray[i]['value'] == "" || formArray[i]['value'] == undefined)) {
						var skip = true;
					}
					if (skip == false) {
						//if (result){
						if (formArray[i].value == "" || formArray[i].value == undefined) {
							...
							else if (formArray[i].type.toLowerCase() == "datetime-local" || formArray[i].type.toLowerCase() == "date") {
								var dateValue = new Date(formArray[i]['value']).toISOString();
								attributeString += "<" + formArray[i]['name'] + "><![CDATA[" + dateValue + "Z]]></" + formArray[i]['name'] + ">";
							}
							else {
				...
				if (formArray[i]['name'] != this.fidField && formArray[i]['name'] != this.geometryField) {
					...
					if (skip == false) {
						...
						if (formArray[i].value == "" || formArray[i].value == undefined) {
							...
							var dateValue = new Date(formArray[i]['value']).toISOString();
							attributeString += "<wfs:Property><wfs:Name>" + formArray[i]['name'] + "</wfs:Name><wfs:Value><![CDATA[" + dateValue + "]]></wfs:Value></wfs:Property>";
						}
						else {
							if (formArray[i].type.toLowerCase() == "number") {
  ...
  xsi:schemaLocation="http://www.opengis.net/wfs http://schemas.opengis.net/wfs/1.0.0/WFS-transaction.xsd http://www.openplans.org/topp">`;
	...
	getEditPopupForm(featureProperties) {
		...
		var htmlForm = '<h2>Edit Feature - ' + this.displayName + '</h2><form id="editAttributesForm" action="nada" onsubmit="return false">';
		var xmlDoc = this.describe;
		var s = new XMLSerializer();
		var fkJson = this.foreignKeyList;
		var a = xmlDoc.getElementsByTagName("xsd:element");
		for (var i = 0; i < a.length; i++) {
			...
			if (s.serializeToString(node.attributes.name) != this.name) {
				...
				if (this.foreignKeyField(fkJson, curName) == false) {
					...
					if (formType == "hidden") {
						htmlForm += '<input type="' + formType + '" id="' + curName + '" name="' + curName + '" value="' + curValue + '" ' + curRequired + '>';
					}
					else if (formType == "textarea") {
						htmlForm += '<textarea id="' + curName + '" name="' + curName + '" rows="5" cols="45" ' + curRequired + '>' + curValue + '</textarea><br>'
					}
					else {
						htmlForm += '<input type="' + formType + '" id="' + curName + '" name="' + curName + '" value="' + curValue + '" ' + curRequired + '><br>';
					}
				}
				else {
					htmlForm += this.getForeignKeyDropDown(fkJson, curName, curValue, curRequired);
				}
			}
