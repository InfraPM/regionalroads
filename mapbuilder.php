...
width:100%
 ...
 #editMapDiv { height: 100%;
     cursor: pointer;
     clear: both;
width:65%;
float:left;
 }
#optionsTextAreaContainer{
top: 250px;
position: absolute;
}
#optionsDiv{
...
<head>
    ...
    <div id="editMapDiv"></div>    
     <div id="optionsDiv">
     <div id="optionsTextAreaContainer">
     <textarea id="options" rows="25" cols="60"></textarea><br>
     <button type="button" id="renderMap">Render Map</button>
     </div>
     </div>
<div id ="popupDiv"></div>
     </div>
    <script>     
     $(document).ready(function() {
         $("#renderMap").click(renderMap);
         function renderMap(){
             var editMap = undefined;
             var appToken = new AppToken();
             appToken.check().then(msg=>{
                     var token = appToken.token;
                     var baseAPIURL = "<?php echo $baseAPIURL; ?>";	
                     var editMapOptions = eval($('textarea#options').val());
                     ...
                     $("#titleContainer h4").html(editMapOptions.title);
                         editMap = new EditMap(appToken, "editMapDiv", editMapOptions);
                     });	     
         }
     });
